...
for epoch in range(5000):# we run 10000 epochs
    for i in full_order:
        ...
        for j in range(1,5):
            hidden_activation[j] = np.tanh(hidden_output[j-1])#1X5 vector(first element is the bias), these serve as inputs for the output layer
        output = np.dot(hidden_activation,b)#this is a 1X3 matrix
        final_output = np.array([0,0,0],dtype = 'float')
        for j in range(3):
            final_output[j] = np.tanh(output[j]) #1X3 vector, this is the final_output
        ...
        output_loc_grad = np.multiply(np.multiply((des - final_output),(np.array([1,1,1]) + final_output)),(np.array([1,1,1])-final_output))#1X3 vector
        ...
        hidden_loc_grad = np.multiply(np.multiply(np.dot(output_loc_grad,bT),np.array([1,1,1,1,1]) + hidden_activation),(np.array([1,1,1,1,1]) - hidden_activation))#1X5 vector
...
#this is for plotting the error trajectory
    if epoch%50 == 0:
        ...
        for t in testing:
            ...
            output = np.dot(hidden_activation,b)#this is a 1X3 matrix
            final_output = np.array([0,0,0],dtype = 'float')
            #print(fina    desired.append([1,0,0])
    if data.iloc[i,2] == 'Iris-versicolor':
        desired.append([0,1,0])
    if data.iloc[i,2] == 'Iris-virginica':
        desired.append([0,0,1])l_output)
            for j in range(3):
                final_output[j] = 1/(1 + np.exp(-output[j])) #1X3 vector, this is the final_output
            if np.argmax(des) != np.argmax(final_output):
                error = error + 1
